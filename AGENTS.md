...
> **Note**: This project has been rebuilt as a Data Analyst Portfolio. The original template structure below has been superseded. See the Portfolio section for current architecture.

## Portfolio Architecture (Current)

All portfolio content lives in `src/routes/index.tsx` which composes section components from `src/components/portfolio/`.

### Component Map

| File | Section |
|------|---------|
| `LoadingScreen.tsx` | Animated spinner shown on first page load |
| `Navbar.tsx` | Fixed nav, dark mode toggle, active section tracking via IntersectionObserver |
| `Hero.tsx` | Typing animation, stat counters (useCounter), CTA buttons, social links |
| `About.tsx` | Bio, profile image with fallback, skill highlights |
| `Skills.tsx` | Animated progress bars per category (useInView triggers width transition) |
| `Projects.tsx` | Hover-overlay project cards with GitHub/demo links |
| `DashboardShowcase.tsx` | BI dashboard gallery with mock bar chart previews |
| `Experience.tsx` | Vertical timeline for education & work |
| `Certifications.tsx` | Certification cards with credential links |
| `Resume.tsx` | Mock resume preview + download/view buttons |
| `Contact.tsx` | Contact info + Netlify Forms AJAX contact form |
| `Footer.tsx` | Social links, nav repeat, back-to-top |

### Custom Hooks

| Hook | Purpose |
|------|---------|
| `useInView` | IntersectionObserver — fires once when element enters viewport |
| `useTypingAnimation` | Typewriter with delete/cycle through word array |
| `useCounter` | Animates 0 → target number over a duration |
| `useTheme` | Dark/light toggle stored in localStorage, sets `.dark` on `<html>` |

### Key Conventions

- **Animations**: Gate on `inView` boolean → apply/remove Tailwind animation class. Defined as `@keyframes` in `styles.css`.
- **Glassmorphism**: `.glass-card` utility class (backdrop-blur + translucent bg). Dark variant in styles.css.
- **Gradient text**: `.gradient-text` class (indigo → violet → cyan). `.gradient-bg` for filled backgrounds.
- **Contact form**: `fetch('/contact-form.html')` with URL-encoded body — Netlify intercepts before SSR. `public/contact-form.html` must mirror all form fields.
- **Dark mode**: `dark` class on `<html>`. All `dark:` variants use the `@custom-variant dark (&:is(.dark *))` rule from styles.css.

### Content to Replace

- `[Your Name]` → in Hero.tsx, Footer.tsx, `__root.tsx`
- `your@email.com` → About.tsx, Hero.tsx, Contact.tsx, Footer.tsx
- GitHub/LinkedIn URLs in Hero.tsx, Contact.tsx, Footer.tsx
- `public/headshot-on-white.jpg` → real profile photo
- `public/resume.pdf` → actual resume (add this file)
- Project cards in Projects.tsx, certification links in Certifications.tsx

---

This document provides an overview of the original project structure for developers and AI agents working on this codebase.
